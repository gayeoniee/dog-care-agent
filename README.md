...
## 같은 문제를 다르게 막은 쪽 — 팀 프로젝트(DAENGS)

같은 피부 모델이 팀 백엔드([SAJOYO/DAENGS_dev](https://github.com/SAJOYO/DAENGS_dev))
에도 들어가 있고, **거기도 프롬프트가 아니라 코드로 막는다** — 이 저장소가
그 아이디어를 처음 낸 게 아니다. 다만 막는 자리가 다르다.

| | DAENGS | 이 저장소 |
|---|---|---|
| 사진 | 앱이 찍고 `/screen/v1/screen`을 **직접** 부른다. 오케스트레이터는 **판정 기록**을 해설 | 오케스트레이터가 MCP 툴로 **직접 호출** |
| 모델이 보는 판정 | `verdict` + `days_ago` **둘뿐** (`ScreeningContext`) | verdict + 계열 + 확률 (6종 이름만 제거) |
| 이름 방어 | **계약에 칸이 아예 없음** + `speaks_beyond_screening`이 문장 교체 | `_for_model`이 빼고 + `G1`이 차단 |
| 위반했을 때 | 그 문장만 고정 문구로 바꾸고 요청은 성공 | 고쳐 쓰게 하고, 또 걸리면 코드가 조립 |
| 사진 + 행동 질문 | 한 턴에 안 합친다 (설계) | 한 턴에서 합친다 |

**DAENGS 쪽이 더 엄격하다.** 계열도 확률도 모델에게 주지 않는다. 저쪽 주석의
말이 정확하다 — *"a field here would demote it to a prompt instruction."*
칸을 없애는 것이 게이트보다 강한 방어다.

이 저장소는 그래서 **개선판이 아니라 다른 트레이드오프**다. 모델에게 더 주고
(계열·확률) 출력 게이트로 갚는 대신, **사진과 근거 검색을 한 턴에서 합칠 수
있다.** 그 교환이 값한 것인지는 `evals/`의 숫자가 말한다.

모달리티 경계를 넘는 값도 적어 둔다. DAENGS에서 같은 걸 하려면
페이로드 타입이 전부 텍스트 전용이고(`SkinPayload`는 사진이 아니라 판정을 받는다),
`_EXECUTE_NAMES`가 `skin`을 일부러 빼고 있으며(명시 신호가 HANDOFF라 넣으면 500),
라우터 벤치마크가 v1~v8로 얼어 있어 라우팅을 건드리면 전부 다시 재야 한다.
**어려워서 안 한 것이 아니라 정해서 안 한 것이고, 그 결정에는 문서가 붙어 있다.**
