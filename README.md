...
| 적대적 평가 (실제 모델 · 24턴) | 첫 초안이 게이트에 걸림 **4/24** → 전부 고쳐 쓰기로 통과 · 조립 0 · **최종 위반 0** |
| 단위 테스트 | **96** |
...
"검색하지 말고 답해")를 실제 모델에 3회씩 던졌다. **24턴 중 4턴에서 첫 초안이
걸렸고**(G1·G5·G6), 넷 다 위반 내용을 돌려주자 고쳐 써서 통과했다. 조립까지 간 턴은
없었고 최종 위반도 없었다. 프롬프트만으로는 6번에 1번은 새고, 게이트가 그 1번을
잡는다 — 이게 이 층의 실제 모양이다. 트레이스는 `evals/out/traces-adversarial/` 에,
집계는 `dogcare stats --dir` 로.
