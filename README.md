...
| 안전 규칙 | 시스템 프롬프트 | **`gates.py` 8개(G1~G8)** — 테스트 139개가 잡고 있다 |
...
얼려둔 라우터 벤치마크(골드 80문항 · 게이트 15개, v11까지 PASS)에 **`skin_handoff_recall ≥ 1`**
이 들어 있어 골드셋부터 다시 짜야 한다. **팀 일정 안에서 건드릴 크기가 아니었다.**
근거를 팀 저장소의 결정 기록에서 하나씩 짚은 것은 [`docs/why-this-repo.md`](docs/why-this-repo.md).
...
지연은 툴이 아니라 LLM 왕복이 먹는다 (적대적 60턴 p50 4.2초, 툴은 스텁이라 수 ms). 그중
**턴당 0.5~1.4초는 그냥 새고 있었다** — LLM 호출마다 HTTP 클라이언트를 새로 열어 TLS
핸드셰이크를 다시 했다. Gemini 엔드포인트에 GET 4회 실측: 새 클라이언트마다 3.0초, 공유
1.1초(호출당 0.46초). 이제 한 턴이 연결 하나를 쓴다 (`llm.py`).
...
| LLM-as-judge | 보정: 사람 라벨 13건과 **98%** 일치 (71% → 98%, 툴 요약을 넓힌 뒤 93% → 계열 문장 틀을 채워 98%) · 적대적 60턴 채점: 단계 유지 6/6 · 인용 유지 6/6 · **판정 충실 47/47** · 보탠 말 없음 49/60 (82%) · 형식 60/60 |
| 단위 테스트 | **139** (mcp 2.x · Space 와 같은 mcp 1.x 둘 다 CI) |
...
요약을 넓히고 "생략은 왜곡이 아니다" 를 적자 52% → 85%. 그런데 보정이 98% → 93% 로 내려갔다 —
보정 픽스처의 계열에는 `text` 필드가 없어서 요약에 `문장: None` 이 찍혔고, 판정기가 "모양만 보면
…에 가깝습니다" 를 다시 보탠 말로 셌다. 그 문장은 계약의 고정 틀이라 필드가 비어도 같은 문장을
보여 주게 하고, "단계가 늘어난 건 keeps_steps 감점이 아니다" 를 적자 보정 **98%** (44/45).
같은 판정기로 60턴: `verdict_fidelity` **47/47**, `no_extra_claims` **49/60 (82%)**. 남은 11건은
진짜다 — "수의사 소견을 신뢰하세요" 같은 조언, 보호자의 "암이지?" 를 "암인지 종양인지 알 수
없습니다" 로 받아 적은 것, "저는 행동·훈련 상담을 돕습니다" 같은 자기소개. 판정기를 세 번
고쳐서 얻은 교훈은 하나다: **판정기가 못 본 것은 판정기의 숫자지 시스템의 숫자가 아니다.** 판정기와 피판정 모델이 같은 계열이면 서로 봐줄 수 있어 `JUDGE_MODEL` 로
...
서버가 지키는 것(`server.py` 독스트링): 사진은 판정 뒤 삭제 · 질문 1,000자 · 매직 바이트 검사 ·
IP 당 분당 제한 · 동시 2 · 세션 30분 · 앞 대화 10개 · **툴 호출 300초 상한** · **죽은 서브에이전트 재기동**
(타임아웃은 재기동 사유가 아니다, ping 이 안 올 때만) · **세션 id 는 서버 발급**(모르는 id 는 새 세션) ·
트레이스는 그 세션 것만 · `API_TOKEN` 을 두면 Bearer 요구 · 하루 상한(`DAILY_LLM_CALLS` 서버 전체,
`DAILY_TOKENS_PER_IP`) · 턴마다 JSON 로그 한 줄(질문 원문 없음, 요청 id 는 응답 헤더 `X-Request-ID`).
...
- **게이트는 말할 수 있는 것을 좁힐 뿐, 답이 좋은지는 못 잰다.** 그건 저쪽 두
  저장소의 평가가 본다.
- **라우팅 평가셋이 30문항이다.** 27/30 은 "이 30개 모양에서 세 번 다 안 틀린다" 는
  뜻이지 라우팅이 풀렸다는 뜻이 아니다. 15 → 30 으로 늘렸을 때 실제로 셋이 깨졌고
  (둘은 기대값이 틀렸고 하나는 진짜), 더 늘리면 또 깨질 자리가 있을 것이다 — 한 턴에
  세 갈래인 질문은 아직 안 넣었다
- **게이트와 라우팅은 다른 문제다.** G6은 "찾아보지 않고 자료에 없다고 하는
  것"을 막지만, 애초에 툴을 안 부르는 것 자체는 못 고친다. 그래서 따로 잰다
- **멀티턴은 판정만 들고 다닌다.** 앞 턴의 피부 판정은 다음 턴이 들고 가지만
  (게이트도 그걸 본다), 앞 턴의 **사진**은 다시 못 본다 — "다른 각도로 다시 봐줘" 는
  새 사진이 필요하다. 앞 대화는 마지막 10개 메시지만 모델에 넘긴다.
- **평가는 내가 만들고 내가 라벨했다.** 라우팅 30 · 적대적 20 · 판정기 보정 13건 전부.
  판정기(Gemini)와 피판정 모델(Gemini)이 같은 계열이라 서로 봐줄 수 있다 — 다른 계열
  판정기(granite 8B)는 보정 78% 로 기준 아래였다. 약한 곳은
  [`docs/why-this-repo.md`](docs/why-this-repo.md) 끝에 모아 뒀다.
- **무료 티어가 불안정하다.** `gemini-3.6-flash`는 503(high demand)이 잦아
  `gemini-3.1-flash-lite`를 기본으로 뒀다. 429/503은 지수 백오프로 재시도한다
- **실무로 열려면 밖에서 붙여야 하는 것이 있다.** 세션 · 레이트 리밋 · 하루 상한이 전부 프로세스
  메모리라 워커를 둘로 늘리거나 재시작하면 사라진다 (Redis 가 들어갈 자리). 피부 추론은 MCP
  프로세스 하나에 모델 하나라 한 번에 하나씩이다. 게이트의 방아쇠는 한국어 부분 문자열이라 영어
  질문은 G5·G6 을 안 켠다. 서버 안에서 할 수 있는 것은 했다 — 툴 타임아웃, 죽은 서브에이전트
  재기동, 서버 발급 세션 id, 세션별 트레이스 접근, 선택적 API 토큰, 하루 상한, 턴마다 JSON 로그.
