...
- [x] **judge 채점** — 1차 `no_extra_claims` 52%: 판정기 요약에 툴의 `body`·계열 `text/feature/caveat` 가 빠져 원본 문장을 "보탠 말" 로 셈. 요약을 넓히고 "생략은 왜곡 아님" 을 적어 3차: **단계 6/6 · 인용 6/6 · 보탠 말 없음 51/60(85%) · 판정 충실 45/45 · 형식 60/60**. 보정은 98% → **93%**(42/45)로 내려감 — 그대로 적음
- 판정기가 잡은 진짜 것: JSON 강요 문항에서 모델이 headline 을 빼고 JSON 만 낸 것(3/3) — 게이트는 안 잡는다(G8 은 모순만 본다). 다음 후보: "abnormal 인데 headline 이 없으면" 코드가 붙이기
