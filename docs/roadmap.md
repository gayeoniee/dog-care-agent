...
# 4차 — 성능 · 인프라 · "왜 이 저장소인가" (2026-09-24)

"더 할 거 있는지 찾아봐 — 성능과 인프라도, 포트폴리오로 서는 이유도" 로 시작한 감사.

- [x] **조립 경로가 계열 dict 를 그대로 찍던 것** — `_compose` 가 `stage2.group`(dict) 을 f-string 에
  넣어 `모양만 보면 {'name': …, 'prob': 0.8, …} 계열에 가깝습니다` 가 나갔다. 진짜 계약이면 `labels` 는
  `_for_model` 이 이미 뗐지만 dict 는 dict 다. 적대적 60턴의 조립 1건이 판정 없는 턴이라 안 드러났다.
  계약의 `text`·`feature`·`caveat` 를 그대로 쓰고 없으면 이름으로 틀을 채운다. 테스트 2개
- [x] **LLM 호출마다 TLS 핸드셰이크** — `httpx.AsyncClient` 를 호출마다 새로 열었다. Gemini 에 GET 4회
  실측: 새 클라이언트마다 **3.0초 · 공유 1.1초**(호출당 0.46초). 턴당 2~4회 부르니 0.5~1.4초/턴.
  턴 동안 하나를 쓰고 끝에 `aclose()`. 테스트 2개
- [x] **켜 둔 동안 계속 차는 표 셋** — `hits`(IP 별 deque, 비어도 안 지움) · `jobs`(SSE 를 안 열면 영영 남음) ·
  세션 `history`(30분 동안 자라서 프롬프트가 턴마다 커짐). 요청마다 치우고, 앞 대화는 마지막 10개
  메시지만(`HISTORY_MAX_MESSAGES`). 테스트 3개
- [x] **`X-Forwarded-For` 첫 값은 지어낼 수 있다** — 고치지 않고 독스트링에 적음. HF 프록시가 덮어쓰는지
  붙이는지 바깥에서 알 수 없고, 우회해서 얻는 게 남의 무료 쿼터뿐이다. 진짜 상한은 `MAX_CONCURRENT` 와 하루 500회
- [x] **Space 가 48시간 뒤 잠드는 것** — `.github/workflows/keepalive.yml` 이 하루 두 번 `/api/health` 를 부른다.
  포트폴리오 링크를 누른 사람이 1~2분을 안 기다리게. GitHub 은 60일 커밋 없으면 schedule 을 끈다(파일에 적음)
- [x] **CI 가 lock 드리프트를 못 보던 것** — `uv sync --locked`
- [x] **README 낡은 한계** — "15문항뿐" → 30, "멀티턴이 얕다"(B4 로 이미 해결) → 사진은 못 들고 다닌다로.
  라우터 벤치마크 "v1~v8" → 실제는 v11 · 골드 80 · 게이트 15 · `skin_handoff_recall ≥ 1`
- [x] **`docs/why-this-repo.md`** — 팀에서 이 칸이 비어 있던 이유를 팀 저장소 `origin/dev` 의 결정 기록에서
  하나씩 인용(D-023 · D-036 · D-079 · D-081, architecture.md, planner.py, 라우터 벤치마크 v11). 그리고 약한 곳.

보지 않은 것: `/api/stats` 가 요청마다 트레이스 폴더를 다 읽는 것(데모 규모에서 문제 안 됨), 툴 병렬 호출(모델이
한 라운드에 툴 둘을 요구하는 턴이 드물고, 스텁은 ms 단위라 잴 것이 없다 — 진짜 모드에서 재면 다르다).

## 4차 이어서 — "실무면 손볼 곳" (같은 날)

서버 안에서 할 수 있는 것은 다 했다. 밖의 것(Redis · 인증 프록시 · 추론 병렬)은 README 한계에 적음.

- [x] **툴 호출 타임아웃** — `TOOL_TIMEOUT_SECONDS`(기본 300, 첫 호출이 가중치를 받는다). 넘으면 그 툴만 오류.
  전에는 없었다 — 멈춘 모델 하나가 요청을 영영 매달았고 동시 상한 2 라 두 번이면 서버 전체가 답을 못 했다
- [x] **죽은 서브에이전트 재기동** — 서브에이전트마다 자기 `AsyncExitStack`. 파이프가 닫히면(`ClosedResourceError`
  등) 한 번 다시 띄우고 같은 호출을 이어 간다. 타임아웃은 재기동 사유가 아니다 — ping 이 5초 안에 안 올 때만.
  `restarts` 가 health 에. 테스트 4개(가짜 세션)
- [x] **세션 id 서버 발급** — 96비트. 모르는 id 는 새 세션. 전에는 클라이언트 8자리를 그대로 열쇠로 써서 남의
  대화 위에서 답할 수 있었다. 작업 id 도 같이
- [x] **트레이스 접근 제어** — `/api/trace/{name}?session_id=` 가 그 세션이 만든 것만. 화면도 그렇게 부른다
- [x] **선택적 인증** — `API_TOKEN` 이 있으면 ask · trace · stats 가 Bearer 요구. events 는 작업 id 가 비밀이라 안 잠금
- [x] **하루 상한** — `DAILY_LLM_CALLS`(서버 전체, Space 450) · `DAILY_TOKENS_PER_IP`(Space 150k). 날짜는 태평양
  자정(Gemini 리셋). Windows 에 tz 데이터가 없어 `tzdata` 의존성 추가
- [x] **구조화 로그 + 요청 id** — 턴마다 JSON 한 줄(지연 · 왕복 · 토큰 · 게이트 · 툴 ms · 재기동). 질문 원문은 안
  남긴다. `X-Request-ID` 헤더와 결과 JSON 의 `request_id`
